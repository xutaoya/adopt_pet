...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">宠物认领管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>宠物认领管理</span></li>
								<li class="breadcrumb-item active"><span>宠物认领列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">宠物认领列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									...
									<input type="text" id="gerenxingmingSearch" class="form-control form-control-sm" placeholder="请输入个人姓名" aria-controls="tableId">
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('gerenzhanghao')">个人账号<i id="gerenzhanghaoIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('gerenxingming')">个人姓名<i id="gerenxingmingIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('lianxishouji')">联系手机<i id="lianxishoujiIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			...
			if($('#gerenxingmingSearch').val() != null && $('#gerenxingmingSearch').val() != ''){
				searchForm.gerenxingming ="%" +  $('#gerenxingmingSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("chongwurenling/page","GET",{
				...
				gerenxingming : searchForm.gerenxingming,	
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item,number){
			...
			var tezhengCell  = document.createElement('td');

						  if(item.tezheng != null && item.tezheng != "" && item.tezheng.length >= 11){
			  	tezhengCell.innerHTML = item.tezheng.substring(0, 10) + "..."; 
			  }else{
			  	tezhengCell.innerHTML = item.tezheng;
			  }


			row.appendChild(tezhengCell);
			...
			var gerenzhanghaoCell  = document.createElement('td');

						 gerenzhanghaoCell.innerHTML = item.gerenzhanghao;

			row.appendChild(gerenzhanghaoCell);
			var gerenxingmingCell  = document.createElement('td');

						 gerenxingmingCell.innerHTML = item.gerenxingming;

			row.appendChild(gerenxingmingCell);
			var lianxishoujiCell  = document.createElement('td');

						 lianxishoujiCell.innerHTML = item.lianxishouji;

			row.appendChild(lianxishoujiCell);
			...
			var crossBtn0 = document.createElement('button');
            		crossBtn0.innerHTML = "认领"
			crossBtn0.setAttribute("type","button");
			crossBtn0.setAttribute("class","btn btn-primary btn-sm");
			crossBtn0.setAttribute("onclick","crossTable(" + JSON.stringify(item) + ",'renlingxinxi','','zhuangtai','该宠物已被认领','已认领,未认领'.split(',')[0])");
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("chongwurenling/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function discussList(id) {
            ...
            window.location.href = "../discusschongwurenling/list.jsp"
        }
		//清除会重复渲染的节点
        function clear(){
	...
	function getSum(colName){
		http("chongwurenling"+colName,"GET",{
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"chongwurenling");
