...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">宠物教学管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>宠物教学列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">宠物教学列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									<label>
										教学名称
									</label>
									<input type="text" id="jiaoxuemingchengSearch" class="form-control form-control-sm" placeholder="请输入教学名称" aria-controls="tableId">
									<label>
										类型
									</label>
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('jiaoxuemingcheng')">教学名称<i id="jiaoxuemingchengIcon" class="fa fa-sort"></i></th>
											<th>教学封面</th>
	...
	<script>
		...
		function search(){
			searchForm = { key: ""};
			if($('#jiaoxuemingchengSearch').val() != null && $('#jiaoxuemingchengSearch').val() != ''){
				searchForm.jiaoxuemingcheng ="%" +  $('#jiaoxuemingchengSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("chongwujiaoxue/page","GET",{
				...
				sort: sortColumn,
				order: sortOrder,
				jiaoxuemingcheng : searchForm.jiaoxuemingcheng,	
		...
		function setDataRow(item,number){
			...
			var jiaoxuemingchengCell  = document.createElement('td');

						 jiaoxuemingchengCell.innerHTML = item.jiaoxuemingcheng;

			row.appendChild(jiaoxuemingchengCell);
			var jiaoxuefengmianCell  = document.createElement('td');

				                var jiaoxuefengmianImg = document.createElement('img');
			jiaoxuefengmianImg.setAttribute('src',baseUrl+item.jiaoxuefengmian);
			jiaoxuefengmianImg.setAttribute('height',100);
			jiaoxuefengmianImg.setAttribute('width',100);
			jiaoxuefengmianCell.appendChild(jiaoxuefengmianImg);


			row.appendChild(jiaoxuefengmianCell);
			var leixingCell  = document.createElement('td');

						  leixingCell.innerHTML = item.leixing; 

			row.appendChild(leixingCell);
			var jiaoxuejianjieCell  = document.createElement('td');

						  if(item.jiaoxuejianjie != null && item.jiaoxuejianjie != "" && item.jiaoxuejianjie.length >= 11){
			  	jiaoxuejianjieCell.innerHTML = item.jiaoxuejianjie.substring(0, 10) + "..."; 
			  }else{
			  	jiaoxuejianjieCell.innerHTML = item.jiaoxuejianjie;
			  }


			row.appendChild(jiaoxuejianjieCell);
			var jiaoxueneirongCell  = document.createElement('td');

			
			var jiaoxueshipinCell  = document.createElement('td');

						  var jiaoxueshipinMedia = document.createElement('button');
			  jiaoxueshipinMedia.setAttribute('class','btn-table')
			  jiaoxueshipinMedia.innerHTML = '播放';
			  var jiaoxueshipinAttr = "mediaPlay('" + baseUrl+ item.jiaoxueshipin + "')"
			  jiaoxueshipinMedia.setAttribute('onclick',jiaoxueshipinAttr);
       		          jiaoxueshipinCell.appendChild(jiaoxueshipinMedia);


			row.appendChild(jiaoxueshipinCell);
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("chongwujiaoxue/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function discussList(id) {
            ...
            window.location.href = "../discusschongwujiaoxue/list.jsp"
        }
		//清除会重复渲染的节点
        function clear(){
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"chongwujiaoxue");
